...
func (s *FavoritesStore) saveLocked() error {
    ...
    if err := shared.EnsureDir(filepath.Dir(s.path)); err != nil {
        return err
    }
    return os.WriteFile(s.path, raw, 0o644)
}

func (s *FavoritesStore) ListFolders() []Folder {
    s.mu.RLock()
    defer s.mu.RUnlock()
    out := make([]Folder, 0, len(s.folders))
    for id, name := range s.folders {
        out = append(out, Folder{ID: id, Name: name})
    }
...
func (s *FavoritesStore) AddFolder(name string) (Folder, error) {
    s.mu.Lock()
    defer s.mu.Unlock()
    id := shared.SHAID("folder", name)
    for {
        if _, exists := s.folders[id]; !exists {
            break
        }
        id = shared.SHAID(id, name)
    }
    s.folders[id] = name
