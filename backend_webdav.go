...
func newWebDAVBackend(lib LibraryConfig, cacheDir string) (Backend, error) {
...
func (b *webdavBackend) Connect(context.Context) error { return ensureDir(b.cacheDir) }

func (b *webdavBackend) buildURL(rel string) string {
    cp := relJoin(b.basePath, rel)
    clone := *b.baseURL
...
func (b *webdavBackend) ListDir(ctx context.Context, rel string) ([]Entry, error) {
    ...
    out := []Entry{}
    current := strings.TrimSuffix(path.Base(strings.TrimSuffix(cleanRel(rel), "/")), "/")
    for _, item := range ms.Responses {
        ...
        entry := Entry{Name: name, RelPath: relJoin(rel, name)}
...
func (b *webdavBackend) ReaderAt(ctx context.Context, rel string) (io.ReaderAt, io.Closer, int64, error) {
    cacheName := filepath.Join(b.cacheDir, shaID(rel)+filepath.Ext(rel))
    if _, err := os.Stat(cacheName); err != nil {
        ...
        if err := copyFile(cacheName, res.Body); err != nil {
            return nil, nil, 0, err
        }
    }
