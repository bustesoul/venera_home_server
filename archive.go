...
func openArchive(ctx context.Context, backend Backend, rel, cacheDir string) (Archive, error) {
...
func materializeArchiveSource(ctx context.Context, backend Backend, rel, cacheDir string) (string, string, int64, error) {
	...
	key := shaID(cleanRel(rel), strconv.FormatInt(size, 10))
	target := filepath.Join(cacheDir, "archive-source", key+strings.ToLower(path.Ext(rel)))
	if _, err := os.Stat(target); err == nil {
		return target, key, size, nil
	}
	if err := copyFile(target, io.NewSectionReader(readerAt, 0, size)); err != nil {
		return "", "", 0, err
	}
	return target, key, size, nil
}
...
func openZIPArchive(ctx context.Context, backend Backend, rel string) (Archive, error) {
	readerAt, closer, size, err := backend.ReaderAt(ctx, rel)
	if err != nil {
		return nil, err
	}
...
func openSevenZipArchive(ctx context.Context, backend Backend, rel string) (Archive, error) {
	readerAt, closer, size, err := backend.ReaderAt(ctx, rel)
	if err != nil {
		return nil, err
	}
...
func openRARArchive(ctx context.Context, backend Backend, rel, cacheDir string) (Archive, error) {
