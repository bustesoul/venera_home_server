...
type WritableBackend interface {
	Backend
	WriteSmallFile(context.Context, string, []byte) error
...
func (b *localBackend) abs(rel string) string {
	rel = strings.ReplaceAll(shared.CleanRel(rel), "/", string(filepath.Separator))
	if rel == "" {
		return b.root
	}
...
func (b *localBackend) ListDir(_ context.Context, rel string) ([]Entry, error) {
	...
	for _, item := range items {
		...
		out = append(out, Entry{
			Name:    item.Name(),
			RelPath: shared.RelJoin(rel, item.Name()),
			IsDir:   item.IsDir(),
			Size:    info.Size(),
			ModTime: info.ModTime(),
		})
	}
	sort.Slice(out, func(i, j int) bool { return shared.NaturalLess(out[i].Name, out[j].Name) })
	return out, nil
}
...
func (b *localBackend) WriteSmallFile(_ context.Context, rel string, data []byte) error {
	path := b.abs(rel)
...
func (b *localBackend) DeleteFile(_ context.Context, rel string) error {
	err := os.Remove(b.abs(rel))
